...
void main() {
  group('BookmarkSortPreferenceTile 위젯 테스트', () {
    ...
    Future<void> pumpWidget(
      WidgetTester tester, {
      required BookmarkDefaultSortType currentType,
    }) async {
      await pumpInhaApp(
        tester,
        child: BlocProvider<UserPreferenceBloc>.value(
          value: bloc,
          child: BookmarkSortPreferenceTile(currentType: currentType),
        ),
      );
      await tester.pump();
    }
    ...
    testWidgets('아이콘과 제목, 현재 설정값을 렌더링한다', (tester) async {
      // Arrange & Act
      await pumpWidget(tester, currentType: BookmarkDefaultSortType.newest);
    ...
    testWidgets('다른 설정값으로 렌더링된다', (tester) async {
      // Arrange & Act
      await pumpWidget(tester, currentType: BookmarkDefaultSortType.oldest);
    ...
    testWidgets('이름순 설정값으로 렌더링된다', (tester) async {
      // Arrange & Act
      await pumpWidget(tester, currentType: BookmarkDefaultSortType.name);
    ...
    testWidgets('다이얼로그에 모든 옵션이 표시된다', (tester) async {
      ...
      // Assert
      expect(find.text('최신순'), findsNWidgets(2)); // 타일 + 다이얼로그
      expect(find.text('과거순'), findsOneWidget);
      expect(find.text('이름순'), findsOneWidget);
      expect(find.byType(RadioListTile<BookmarkDefaultSortType>),
          findsNWidgets(3));
    });
    ...
    testWidgets('다이얼로그에서 현재 선택된 옵션이 체크된다', (tester) async {
      ...
      // Assert
      final radioTiles =
          tester.widgetList<RadioListTile<BookmarkDefaultSortType>>(
        find.byType(RadioListTile<BookmarkDefaultSortType>),
      );

      expect(radioTiles.length, 3);
      expect(
          radioTiles.elementAt(0).groupValue, BookmarkDefaultSortType.oldest);
      expect(radioTiles.elementAt(1).value, BookmarkDefaultSortType.oldest);
    });
  });
}
