...
class GraduateSchoolSettingPage extends StatelessWidget {
    ...
    return BlocProvider(
      create: (_) => di.sl<UniversitySettingBloc>()
        ..add(const LoadSettingEvent(
          prefKey: SharedPrefKeys.kGraduateSchoolKey,
          settingType: UniversitySettingType.graduateSchool,
        )),
      child: const _GraduateSchoolSettingView(),
    );
  }
}

class _GraduateSchoolSettingView extends StatelessWidget {
  const _GraduateSchoolSettingView();

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: const CommonAppBarWidget(
          title: '대학원 설정', titleSize: 20, isCenter: true),
